...
- [ ] **Step 9: Roll out the new module entry point to every remaining content page**
  Update the `<script>` tags across all remaining HTML content pages (all units, Start Here, etc.) to load `js/main.js` (with the correct relative path per folder depth) instead of `js/loadmathjax.js`. Once every page has been updated and confirmed working, delete `js/loadmathjax.js` and remove any remaining references to it. While you're in each page for this update, apply the accessibility and container constraints above (fix obvious heading-order skips, add image captions where missing) since you're already touching the file.
